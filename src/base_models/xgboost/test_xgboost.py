...
from src.general_helpers.data_pipeline import create_xgboost_splits
...
def run_xgboost_finetuning(
    ...
    for fold_index, (fit_idx, holdout_idx) in enumerate(split.kfold_indices, start=1):
        fold_model = XGBoostFraudDetector(config or XGBoostConfig(random_seed=random_seed + fold_index))
    ...
    final_model = XGBoostFraudDetector(config or XGBoostConfig(random_seed=random_seed))
...
def save_xgboost_outputs(result: dict, output_dir: Path) -> None:
    ...
    np.save(output_dir / "xgb_train_oof.npy", result["train_oof"])
    np.save(output_dir / "xgb_val_probs.npy", result["eval_probs"])
    np.save(output_dir / "xgb_test_probs.npy", result["test_probs"])
